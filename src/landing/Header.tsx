          ...
          {navItems.map((item) => {
            ...
            return (
              <a
                ...
                className={`text-[14px] font-['Space_Grotesk'] transition-all duration-300 ${
                  isActive
                    ? "flex items-center justify-center bg-white border border-[#f0f1f2] rounded-[39px] py-[9px] px-[7px] text-[#041133] font-medium shadow-sm"
                    : "py-[8px] px-[10px] text-[#9ca1aa] hover:text-[#041133]"
                }`}
              >
                {item.label}
              </a>
          ...
          {navItems.map((item) => {
            ...
            return (
              <a
                key={item.id}
                href={item.href}
                className={`text-[18px] font-['Space_Grotesk'] py-2 border-b border-gray-100 transition-colors ${
                  isActive ? "text-[#041133] font-medium" : "text-[#646a77]"
                }`}
                onClick={() => handleNavItemClick(item.id)}
              >
                {item.label}
